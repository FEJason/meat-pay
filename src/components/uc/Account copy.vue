                ...
                <section class="accountContent">
                    <div class="account-in">
                        ...
                        <div class="account-item">
                            <div class="account-item-in u-flex u-row-between">
                                <div class="u-flex">
                                    ...
                                    <p v-if="user.bankVerified == 1" class="bankInfo" style="color: #fff;">
                                        {{user.bankInfo.account}}，{{user.bankInfo.accountName}}，{{user.bankInfo.accountInfo}}
                                    </p>
                                ...
                                <div>
                                    <!-- 绑定 -->
                                    <a class="btn" v-if="user.bankVerified == 1" @click="showDialog = true">{{$t('uc.account.modify')}}</a>
                                    <!-- 修改 -->
                                    <a class="btn" v-else @click="modify('bank')">{{$t('uc.account.bind')}}</a>
                                </div>
                            </div>
                            <Modal
                                class="detail"
                                ...
                                <div class="account-detail">
                                    <div class="detail-list">
                                        <Form ref="formValidate1" :model="formValidate1" :rules="ruleValidate" :label-width="85">
                                            <!-- 姓名 -->
                                            <FormItem :label="$t('uc.account.name')" prop="kycName">
                                                <Input disabled size="large" v-model="formValidate1.kycName"></Input>
                                            </FormItem>
                                            <!-- 开户银行 -->
                                            <FormItem :label="$t('uc.account.bankaccount')" prop="accountName">
                                                <Select v-model="formValidate1.accountName" size="large" :placeholder="$t('common.pleaseselect')">
                                            ...
                                            <FormItem :label="$t('uc.account.bankbranch')" prop="accountInfo">
                                                <Input v-model="formValidate1.accountInfo" size="large"></Input>
                                            </FormItem>
                                            <!-- 银行账号 -->
                                            <FormItem :label="$t('uc.account.bankno')" prop="account">
                                                <Input v-model="formValidate1.account" size="large" type="text"></Input>
                                            </FormItem>
                                            <!-- 确认卡号 -->
                                            ...
                                            <FormItem class="u-text-right">
                                                <Button type="text" @click="showDialog = false" class="u-m-r-10">取消</Button>
                        ...
                        <div class="u-flex u-row-right u-p-r-30">
                            <Button type="primary" icon="md-add">添加收款方式</Button>
                        </div>
                    </div>
                </section>
            </div>
...
<script>
import { getPaymentList, savePayment, getPayType } from '@/api/user'
export default {
    components: {
    },
    data() {
    ...
    methods: {
        ...
        submit(name) {
            ...
            if (name == 'formValidate1') {
                console.log(JSON.parse(JSON.stringify(this.formValidate1)))
                return
                let param = {}
...
::v-deep .ivu-modal-header p, ::v-deep.ivu-modal-header-inner {
    color: #fff !important;
}

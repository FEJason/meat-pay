  ...
  <div class="entrustcurrent">
    <Form class="form" :model="formItem" :label-width="75" inline>
      ...
      <FormItem :label="$t('uc.finance.trade.direction')">
        <Select v-model="formItem.direction" style="width:70px;" :placeholder="$t('common.pleaseselect')">
          <Option value="BUY">{{$t('uc.finance.trade.buy')}}</Option>
          <Option value="SELL">{{$t('uc.finance.trade.sell')}}</Option>
        </Select>
      </FormItem>
      <FormItem>
    ...
    <div class="table">
      <Table :no-data-text="$t('common.nodata')" :columns="columns " :data="orders" :loading="loading">
        <template slot-scope="{ row, index }" slot="slotCancel">
          <Button type="ghost" @click="cancel(row.orderId)" size="small">{{ $t("exchange.undo") }}</Button>
        </template>
      </Table>
      <div class="page">
...
import { currentOrder } from '@/api/user'
    ...
    return {
      ...
      columns: [
        ...
        // },
        {
          ...
          minWidth: 55,
          render: (h, params) => {
            return h("span", {}, this.dateFormat(Number(params.row.time)));
          }
        },
        {

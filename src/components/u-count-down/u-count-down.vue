<template>
	<div class="u-countdown">
		<div class="u-countdown-item" :style="[itemStyle]" v-if="showDays && (hideZeroDay || (!hideZeroDay && d != '00'))">
			<div class="u-countdown-time" :style="[letterStyle]">
				{{ d }}
			</div>
		</div>
		<div
			class="u-countdown-colon"
			:style="{fontSize: separatorSize + 'px', color: separatorColor, paddingBottom: separator == 'colon' ? '4px' : 0}"
			v-if="showDays && (hideZeroDay || (!hideZeroDay && d != '00'))"
		>
			{{ separator == 'colon' ? ':' : '天' }}
		</div>
		<div class="u-countdown-item" :style="[itemStyle]" v-if="showHours">
			<div class="u-countdown-time" :style="{ fontSize: fontSize + 'px', color: color}">
				{{ h }}
			</div>
		</div>
		<div
		...
		<div class="u-countdown-item" :style="[itemStyle]" v-if="showMinutes">
			<div class="u-countdown-time" :style="{ fontSize: fontSize + 'px', color: color}">
				{{ i }}
			</div>
		</div>
		...
		<div class="u-countdown-item" :style="[itemStyle]" v-if="showSeconds">
			<div class="u-countdown-time" :style="{ fontSize: fontSize + 'px', color: color}">
				{{ s }}
			</div>
		</div>
 ...
 * @property {String Number} separator-size 分隔符的字体大小，单位px（默认30）
 * @property {String} separator-color 分隔符的颜色（默认#303133）
 * @property {String Number} font-size 倒计时字体大小，单位px（默认30）
 ...
 * @property {String} height 数字高度值(宽度等同此值)，设置边框时看情况是否需要设置此值，单位px（默认auto）
